...
const Toggle = ({ label, description, isEnabled, onToggle }) => (
    <SettingItem>
        <div className="flex justify-between items-center">
            ...
            <button
                onClick={onToggle}
                className={`relative w-14 h-8 rounded-full transition-colors duration-300 ${isEnabled ? 'bg-primary' : 'bg-black/30'}`}
...
export const SettingsModal = ({ isOpen, onClose }) => {
  ...
  // Map ACCENT_COLORS to format expected by ColorPicker component
  const colorOptions = ACCENT_COLORS.map(c => ({
    value: c.value,
    label: c.labelFull || c.label,
    hex: c.value,
  }));
